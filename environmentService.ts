		...
		if (this.payload) {
			...
			if (fileToOpen) {
				...
				if (this.payload.has('gotoLineMode')) {
					const gotoLineMode = this.payload.get('gotoLineMode') || '';
					if (gotoLineMode.includes(':')) {
						const range = gotoLineMode.split(',');
						const start = range[0].split(':');
						let end = start;
						if (range.length > 1) {
							end = range[1].split(':');
						}
						const selection = { startLineNumber: parseInt(start[0]), startColumn: parseInt(start[1]) + 1,
							endLineNumber: parseInt(end[0]), endColumn: parseInt(end[1]) + 1 };
						const filesToOpen: IPath<ITextEditorOptions>[] = [];
						if (fileUri.scheme !== 'manifest') {
							filesToOpen.push({
								fileUri: fileUri,
								options: {
									selection: selection
